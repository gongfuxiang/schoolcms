...
class NavHeaderController extends CommonController
{
	...
	private function GetNavList()
	{
		...
		$data = $this->DataDealWith($m->field($field)->where(array('nav_type'=>'header', 'pid'=>0))->order('sort')->select());
		if(!empty($data))
		{
			foreach($data as $k=>$v)
			{
				$data[$k]['item'] = $this->DataDealWith($m->field($field)->where(array('nav_type'=>'header', 'pid'=>$v['id']))->order('sort')->select());
			}
		}
		return $data;
	}
